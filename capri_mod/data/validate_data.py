...
# Unsupported high-value specialty crops (vegetables, fruit, olives) legitimately
# run larger negative *market* margins than staple field crops: they receive no
# coupled CAP support, are grown on small areas, and their CAPRI cost category
# bundles heterogeneous (incl. protected/greenhouse) production. A modest loss
# for these is not a data error the way it would be for a staple, so they use a
# looser artifact threshold. A genuine staple artifact (e.g. the sugar-beet price
# bug) is still caught at the strict threshold above.
_SPECIALTY_MARKET = {"OVEG", "TOMA", "APPL", "OFRU", "CITR", "OLIV",
                     "PULS", "TAGR", "TWIN", "TABO", "WINE"}
_ARTIFACT_LOSS_SPECIALTY = -8000.0
...
def _check_margins(D, rep):
    ...
    try:
        ...
        # Apply the same greenhouse-artifact cost cap the supply loader applies
        # (load_variable_costs_regional), so the margin check evaluates the costs
        # the model actually uses rather than the raw file's greenhouse values
        # misapplied to field-scale specialty crops.
        if c.shape[0] > 50:
            _SPECIALTY = ["OVEG", "TOMA", "APPL", "OFRU", "CITR", "OLIV",
                          "PULS", "TAGR", "TWIN", "TABO"]
            for _crop in _SPECIALTY:
                if _crop in c.columns:
                    _pos = c[_crop][c[_crop] > 0]
                    if len(_pos):
                        _cap = 2.0 * _pos.median()
                        c.loc[c[_crop] > _cap, _crop] = _cap
    ...
    for reg in areas.index:
        ...
        for crop in crops:
            ...
            loss_threshold = (_ARTIFACT_LOSS_SPECIALTY
                              if crop in _SPECIALTY_MARKET else _ARTIFACT_LOSS)
            if full_margin < loss_threshold:
