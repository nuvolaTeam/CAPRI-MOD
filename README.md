...
CAPRI-mod is a **partial-equilibrium model of EU agriculture with a
comparative-static core and a recursive-dynamic projection layer**. Given a
policy or price shock, the core computes how the base-year agricultural economy
re-settles into a new equilibrium: how farmers reallocate land and herds, how
markets clear, and what the environmental consequences are. The projection layer
runs that core forward along an externally-adopted baseline trajectory.

Both halves of that description carry caveats worth reading before relying on
either:

- **Nearly all the validation sits on the comparative-static core.** The
  projection has been validated for a **single step** to 2030, against CAPRI's
  own 2030 reference (correlation +0.997, total cropped area within 2%).
- **Multi-period recursion runs and produces year-specific results, but has not
  been validated against an external reference.** Only perennial crop areas are
  carried between periods; herds are re-optimised each period, which overstates
  livestock adjustment speed.
- **The trajectory is exogenous** — adopted from CAPRI's `captrd` baseline, not
  generated by this model.
- **Partial equilibrium is deliberate**: agriculture is resolved, the wider
  economy is not — the same boundary CAPRI draws (see §11).

**It answers:** *"What does this policy, on its own, do to the agricultural
sector?"* — the policy effect, separated from baseline drift and from
economy-wide feedbacks.

"On its own" means two specific things, and one thing it does **not** mean:

- **Separated from baseline drift.** In a projection, `ProjectionModule.run()`
  solves each target year twice — baseline and scenario — and reports the
  increment separately, so a 2030 result never conflates "the world changing
  anyway" with "what the policy did".
- **Separated from economy-wide feedbacks.** Labour markets, capital and GDP
  effects lie outside the partial-equilibrium boundary — **the same boundary
  CAPRI draws**. CAPRI is also partial-equilibrium and reaches economy-wide
  effects by *linking* to an external CGE (GTAP, via its `capri_gtap` and
  `cgemrk` modules), not by embedding one. We share the boundary and the
  principle; we differ in maturity, since CAPRI has that linkage implemented and
  this model does not.
- **NOT insulated from world markets.** The market module carries 29 trade
  regions with Armington substitution, so an EU policy moves world prices and
  those feed back into EU supply. Leakage is *modelled*, not excluded — the same
...
| `fert/` | 280 | derive N/P/K application rates | removal × yield × calibrated efficiency |
| `abatement/` | 440 | carbon-price MACC + EcAMPA measures | economic reallocation + technological measures |
| `income/` | 200 | farm income & its distribution | gross margin + CAP support, regional Gini |
...
**Fertilizer — deriving application rates.** Distributes fertilizer to crop/region
application rates (CAPRI's `p_FertPerHa`) from nutrient removal factors × yields ×
a group-and-nutrient efficiency factor calibrated to CAPRI. This is the core
allocation CAPRI's fertilizer estimation is built around, without the full
Bayesian solve; it reproduces CAPRI's per-hectare application within ~10%. Its
purpose is to *derive* application rates for a base year that lacks a ready-made
CAPRI file — the fertilizer half of a base-year update — and it feeds the
environmental nitrogen balance (`EnvironmentalModule(data, fertilizer_module=fm)`).
Key class: `FertilizerModule`.

**Abatement — two honest layers.** GHG mitigation, kept as two deliberately
...
### 9.0 Fitness for use — read this first

Validation against CAPRI's own Green Deal 2030 scenario (run on a *shared*
baseline: the trajectory here and CAPRI's scenario reference agree to
`SWHE.YILD.2030 = 8515.98`) established a clear split.

**Validated against CAPRI's own 2030 baseline (projection layer):** projecting to
2030 on the captrd trajectory reproduces CAPRI's 2030 reference to a correlation
of +0.997 on activity levels, total cropped area within 1% (30320 vs 30131 kha),
and 16/24 crops within 25%. Olives are the notable miss (0.35x).

**Sound for:**
- Base-year analysis and the comparative-static policy increment.
- **Area-based instruments** — set-aside / landscape elements, CAP payment rates
  (BISS, coupled support, eco-schemes, Pillar II). Magnitudes are the right
  order against CAPRI.
- The carbon-abatement MACC (validated against EcAMPA 2 at €50/t).
...
- **Nitrogen or mineral-fertiliser ceilings.** Structurally overstated, ~10x.
  Nutrient coefficients are fixed per activity, so the model has no intensity
  margin: it can only cut area, never N per hectare. See §11.
- **Pesticide instruments — not represented.** CAPRI applies the Farm-to-Fork
  pesticide target as a plant-protection input/cost reduction (with no yield
  penalty), which would be replicable — but this model's variable costs are a
  single aggregate per activity with no input breakdown, and the available CAPRI
  cost-component extract deliberately omits PLAP because in those dumps it is a
  quantity index rather than a cost. That warning applies to `NETF.PLAP` (tonnes,
  set equal to `pestotal`); CAPRI also carries **`EAAB.PLAP` in million euro**,
  which is the cost basis, sourced from the Eurostat Economic Accounts for
  Agriculture. The instrument is therefore blocked on an extraction, not on the
  data existing: what is needed is per-activity `PLAP` on the EAAB monetary basis.
- Organic *area targets* — only a payment-rate proxy exists.

The distinction is structural, not a tuning issue, and it will not improve until
N-per-hectare becomes a decision variable with a yield response.
...
Stated plainly, because a defensible model names its limits. These fall into two
kinds: *genuine caveats* to keep in mind when reading results, and *deliberate
scope boundaries* — domains the model does not enter by design, each with a
principled reason and extension path.

**Genuine caveats (mind these when reading results):**
...
- **Supply elasticities** are within ~10% of CAPRI's for major cereals. The
  previously documented sign issue for pulses no longer occurs: measured
  directly, all 12 crops tested show positive own-price elasticities across
  ~690 region-crop checks, with zero negative responses. Pinned by
  `test_no_negative_own_price_elasticities`.
- **No intensity margin: nitrogen/fertiliser-ceiling scenarios are structurally
  overstated.** Nutrient coefficients are fixed per activity, so N per hectare is a
  constant and a nitrogen constraint can only be met by cutting AREA. CAPRI adjusts
  intensity (less N on the same land) and splits the response between the intensity
  and extensive margins. Validated against CAPRI's Green Deal run, CAPRI-mod's wheat
  response to an N ceiling is ~-24% against CAPRI's -1.9%. Area-based instruments
  (set-aside, landscape elements) are sound — 4.0% against CAPRI's ~1.2%, the right
  order. Do not use intensity-based instruments for policy conclusions until
  N-per-hectare becomes a decision variable with a yield response.
- ~~One region (Hamburg, DE60) does not converge~~ — **resolved**. All 248
  regions now converge, DE60 included, with zero QP-solver fallbacks.

**Deliberate scope boundaries (not gaps — design choices, like §10):**

- **Operates at NUTS-2, not sub-regional.** The economic logic is validated at
  the level of the 248 NUTS-2 regions. Disaggregation to farm types or a 1km grid
  (CAPRI's separate farmtype/capdis models) is a *post-processing spatial layer*
  requiring farm-structure (FADN) and land-cover data, not an economic extension
  of this core. Inventing sub-regional detail without that data would manufacture
  false precision — a detailed map reads as authoritative precisely because it is
  detailed, so fabricated downscaling is more misleading than none. The honest,
  data-backed unit of analysis is the region.
- **Partial equilibrium, by design.** The model resolves the agricultural sector;
  it does not close the loop with the wider economy (labour and capital markets,
  GDP feedbacks — CAPRI's link to an external CGE such as GTAP). This is the same
  "link, don't rebuild" principle CAPRI itself follows: economy-wide feedbacks,
  if needed, come from *linking* the validated partial-equilibrium core to an
  external CGE, not from embedding a general-equilibrium model inside it. CAPRI
  is partial-equilibrium too and links to GTAP through its `capri_gtap` and
  `cgemrk` modules. The boundary and the principle are shared; the difference is
  that CAPRI has the linkage **implemented** and this model has only documented
  it as the path. And the boundary is a
  feature: partial equilibrium is exactly what *isolates* the agricultural policy
  effect, which is the model's purpose (§10.1).
- **Income distribution is across regions, not across farms.** The farm-income
  module (§5.2) reports how income and CAP support are distributed across the 248
  regions — a data-backed statement. Distribution across farm sizes or types would
  need FADN farm-structure data the model does not carry, and is deliberately not
  claimed.
- **Agricultural water demand and biodiversity pressure are in-domain indicator
  layers; their environmental *outcomes* are not.** Both pressure sides are now
  built: biodiversity *pressure* proxies (Shannon crop diversity, HNV-farmland
  proxy) in the environmental module, and irrigation *water demand* in the water
  module (CNIR × irrigated area, from CAPRI's CROPWAT data and Eurostat FSS
  irrigation shares; validated against the known Mediterranean concentration of
  EU irrigation). Their environmental *outcomes* remain out of scope: species-level
  population change needs an ecological response model, and catchment *hydrology*
  (whether demand depletes a given aquifer) is a separate modelling domain. The
  pattern is consistent — the agricultural pressure/demand side is in-domain; the
  downstream environmental outcome (species, hydrology) is not.
